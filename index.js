...
let oldTab = userTab;
const API_KEY = "874e457dc636338dc6197eab3f13850e";
...
function switchTab(newTab){
    if(newTab != oldTab){
        ...
        if(!searchForm.classList.contains("active")){
            ...
            //main phle search wale tab me tha, ab your weather tab visible krna h
...
function getfromSessionStrorage(){
    ...
    if(!localCoordinates){
        //agar local storage nhi mile to
        grantAccessContainer.classList.add("active");
       
    }
        else{
            const coordinates = JSON.parse(localCoordinates);
            fetchUserweatherInfo(coordinates);//user ke weather ko fetch krke la rha h
        }
...
async function fetchUserweatherInfo(coordinates){
    ...
    function renderWeatherInfo(weatherInfo){
        ...
        cityName.innerText = weatherInfo?.name;//? for to check wheather value inside exist or not
